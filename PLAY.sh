...
_access () {
	...
	ATK=$(cat SRC | grep -o -P '\W\w{4,12}\W\w{0,4}at\w{0,3}k\W{2}r\W\d{0,10}' | sed -n 1p)
#	ATTACK=$(cat SRC | grep -o -P '\W\w{4,12}\Wat\w{0,3}k\W{2}r\W\d{0,10}')
	ATKRND=$(cat SRC | grep -o -P '\W\w{4,12}\Wat\w{0,3}k\w{3,6}\W{2}r\W\d{0,10}')
...
#	KINGATK=$(cat SRC | sed 's/href=/\n/g' | grep 'king/kingatk/' | head -n1 | awk -F"[']" '{ print $2 }')
	DODGE=$(cat SRC | grep -o -P '\W\w{4,12}\Wd\w{4}\W{2}r\W\d{0,10}')
	HEAL=$(cat SRC | grep -o -P '\W\w{4,12}\Wh\w{3}\W{2}r\W\d{0,10}')
	STONE=$(cat SRC | grep -o -P '\W\w{4,12}\Ws\w{4}\W{2}r\W\d{0,10}')
	GRASS=$(cat SRC | grep -o -P '\W\w{4,12}\Wg\w{4}\W{2}r\W\d{0,10}')
