members_allies () {
 ...
 if [ -n "$CLD" ] ; then
  ...
  for num in `seq 5 -1 1` ; do
   printf "${PURPLEis_BLACK}/clan/${CLD}/${num}\033[00m\n"
   (
...
#   sleep 1s
 ...
 cat allies.txt
 printf "${BLACK_CYAN}Wait to continue.  👈\033[00m\n"
 sleep 5
}
id_allies () {
 printf "${BLACK_CYAN}Looking for allies on friends list\033[00m\n"
 cd $TMP
 printf "${PURPLEis_BLACK}/mail/friends\033[00m\n"
 (
...
clan_allies () {
 ...
 if [ -n "$CLD" ] ; then
  ...
  for num in `seq "$Lnl" -1 "$nl"` ; do
   ...
   if [ -n "$IDN" ] ; then
    printf "${Lnl} ${PURPLEis_BLACK}/user/${IDN}\033[00m\n"
    (
    ...
    printf "${PURPLEi_BLACK} ${LEADPU} - ${alCLAN}\033[00m\n"
    if [ -n "$LEADPU" ] ; then
     ts=$((ts + 1))
     echo $LEADPU | sed 's,\ ,_,' >>callies.txt
     printf "${BLACK_CYAN} ${ts}. Ally ${LEADPU} ${alCLAN} added.\033[00m\n"
